...
The test data for this year's GDI task is split across three files in the following formats:

	test.txt	each line contains the transcript of one utterance
	test.norm	each line contains the transcription text together with word-level normalization, separated by |:
				originalword1|normalizedword1 originalword2|normalizedword2 ...
	test.vec	each line contains a 400-dimensional iVector representing the acoustic features of the utterance;
				the values are separated by spaces

Participants are requested to predict one label (out of BE, BS, LU, ZH) for each utterance/line of the test set. The prediction file should thus contain the same number of lines as test.txt, and each line should contain exactly one label.
